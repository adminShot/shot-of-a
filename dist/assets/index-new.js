...
(() => {
  // bin/live-reload.js
  new EventSource(`${"http://192.168.8.113:3000"}/esbuild`).addEventListener("change", () => location.reload());
  ...
  var cityDetector_func = () => {
    ...
    function setDefaultCity() {
      const locationButtons = document.querySelectorAll("[location-dropdown_button]");
    ...
    async function func_locationApi() {
      try {
        ...
        if (matchedCity) {
          ...
          if (cityGuess)
            cityGuess.textContent = matchedCity;
        } else {
          setDefaultCity();
        }
      } catch (error) {
    ...
    function saveCityToSessionStorage(city) {
      if (sessionStorage.getItem("cityFromApi"))
        return;
    ...
    if (savedCity) {
      ...
      const cityGuess = document.querySelector("[city-guess]");
      if (cityGuess)
        cityGuess.textContent = defaultCity;
      if (urlCity) {
    ...
    const tip = document.querySelector("[city-detector-tip]");
    if (tip) {
    ...
    const locationDropdownButtons = document.querySelectorAll(
